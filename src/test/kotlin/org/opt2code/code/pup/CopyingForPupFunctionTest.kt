...
package org.opt2code.code.pup

import org.junit.Assert
import org.junit.Test
import org.opt2code.code.*

class CopyingForPupFunctionTest {

    inline fun StringFieldProperty.fieldCall(noinline f: Sp<String?>? = null): String? { return fieldBackedByPup(f)}

    inline fun StringFieldProperty.getCall(noinline f: Sp<String?>? = null): String? { return pup(f) { null }}

    inline fun TestData<*>.isSet(name: String) = r.propSet(name)

    inline fun KtLazy.isSet(name: String) = propSet(name)

    val fieldName = StringFieldProperty::fieldBackedByPup.name
